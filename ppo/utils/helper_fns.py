...
def evaluate_agent(envs, model, run_count, seed, greedy_actor=False):
    ...
    while not finish:
        with torch.no_grad():
            actions = model.get_action(next_obs.to("cpu"))#model(next_obs)
        next_obs, rewards, _, info = envs.step(actions.numpy())
...
def record_video(env_id, agent, file, exp_type=None, greedy=False):
    ...
    while not done:
        with torch.no_grad():
            actions = agent.get_action(next_obs)#model(next_obs)
        next_obs, rewards, terminated, info = env.step(actions.detach().numpy())
        next_obs = torch.Tensor(next_obs)
        if terminated:
            done = True
...
def record_video_sac(env_id, agent, file, exp_type=None, greedy=False):
    ...
    while not done:
        with torch.no_grad():
            action,log_prob = agent(env,torch.Tensor(state).to(device),deterministic = False,explore=False)
