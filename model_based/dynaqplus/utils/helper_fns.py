...
def evaluate_agent(envs, model, run_count, seed, greedy_actor=False):
    ...
    while not finish:
        ...
        next_obs, rewards,truncated, terminated, infos = envs.step(actions)
        next_obs = next_obs
...
def record_video(env_id, agent, file, exp_type=None, greedy=False):
    ...
    while not done:
        actions = agent.get_action(np.array([next_obs]))#model(next_obs)
        next_obs, rewards,truncated, terminated, info = env.step(actions[0])
        out = env.render()
        frames.append(out)
    ...
    anim.save(file, writer="ffmpeg", fps=1)
