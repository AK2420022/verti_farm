...
def evaluate_agent(envs, model, run_count, seed,tasks, greedy_actor=False):
    ...
    model.eval()
    next_obs = next_obs[0]["observation"]
    total_reward = 0
    while not finish:
        with torch.no_grad():
            # Initialize Ornstein-Uhlenbeck Process
            noise_process = np.zeros(envs.action_space.shape)
            noise_scale = 0.3
            noisep = edict()
            ...
            noisep.dt = 1e-2
            noisep.mu = 0
            actions = model(next_obs)
            next_obs_, rewards, _,_, infos = envs.step(actions.numpy())
            next_obs = torch.Tensor(next_obs_["observation"])
            total_reward += rewards
            for item in info:
                if "episode" in item.keys():
                    returns_over_runs.append(total_reward)
            ...
            for info,values in infos.items():
                if info == 'step_task_completions' :
                    if np.equal(values, np.array(tasks)).all():

                        returns_over_runs.append(total_reward)
                        episode_len_over_runs.append(item["episode"]["l"])
                    if run_count==len(episode_len_over_runs):
                        finish = True
    model.train()
...
def evaluate_agent_sac(envs, model, run_count, seed, greedy_actor=False):
    ...
    while not finish:
        with torch.no_grad():
            actions,log_prob= model.get_action(envs,next_obs,deterministic = False,explore=False)
        next_obs, rewards, _, info = envs.step(actions.detach().numpy())#.reshape(-1, 1))#
...
def record_video(env_id, agent, file, exp_type=None, greedy=False):
    ...
    """
    frames = []
    env = gym.make(env_id,tasks_to_complete=['microwave', 'kettle'])
    ...
    while not done:
        ...
        noisep.mu = 0
        with torch.no_grad():
            actions = agent(next_obs)
        print("acton eval: ",actions.detach().numpy())
        next_obs, rewards, terminated, info = env.step(actions[0].detach().numpy())
        next_obs = torch.Tensor(next_obs)
        if terminated:
            done = True
...
def record_video_sac(env_id, agent, file, exp_type=None, greedy=False):
    """
    Records one episode of the agent acting on the environment env_id and saving the video to file.
    Args:
        env_id: The environment id for the agent to run in e.g. Cartpole-v1
